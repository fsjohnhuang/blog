...
** Solution One
   ...
   cnpm install sqlite@latest --build-from-source --runtime=electron --target=1.7.9 --dist-url=http://atom.io/download/electron --save
...
*** 配置
    ...
    2. 配置(package.json)
       ...
       {
         "scripts": {
           "pack": "electron-builder --dir", // npm run pack, 生成package目录但不打包为一个文件
           "dist": "electron-builder" // npm run dist, 生成一个exe或dmg文件
         },
         "build": {
	  ...
	  "requestedExecutionLevel": "highestAvailable" // Ask for executing this application as Administrator every time. The value "highestAvailable" is the same as "requireAdminstrator". And the "asInvoker" is the default value.
             },
       "nsis": {
...
**** 双package.json文件进行依赖管理
     作用：用于将package.json中dependencies中的部分依赖排除在打包之外。
     用法：
     1. 在项目下创建app目录
	若根目录下存在app目录，那么electron-builder会以该目录作为打包的根目录，并且只会打包该目录下的文件(即package.json中的build配置均是基于app目录而定)
     2. 将根目录的package.json复制app目录下，并删除devDependencies和dependencies中不被打包的依赖。

     这时开发使用的依赖则写入根目录的package.json，而打包用的则写入app/package.json。
